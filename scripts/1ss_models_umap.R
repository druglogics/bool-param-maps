...
for(n_neighbors in 2:20) {
  ...
  # in case you don't want to re-run the above command and have all the .rds result files
  #lo_umap = readRDS(file = paste0("data/1ss_lo_umap/lo_umap_1ss_", n_neighbors, "nn.rds"))
  ...
  lo_umap %>%
    ...
    ggplot(aes(x = X, y = Y)) +
      geom_point(shape = '.') +
      labs(title = paste0("Parameterization Map - ", n_neighbors, " Neighbours")) +
      theme_classic() +
